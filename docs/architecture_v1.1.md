...
descriptors/
├── fvg_description.yaml
├── asia_session.yaml
└── liquidity_zones.yaml

📌 Обновлено (v1.1): добавлено разделение на static/dynamic/descriptors, наследование от BaseDetector, JSON-валидация через pydantic.
...
setup_manager.py         ← интерпретатор YAML правил, возвращает JSON-контекст Prefect
yaml_rules/
├── ib_raid_v1.yaml
...
📌 Обновлено (v1.1): Setup Manager не управляет Prefect, он только интерпретирует YAML и возвращает JSON с required_tools, symbol, tf.
...
manual_backtest_ui.py   ← Dash-интерфейс ручного теста
recorder.py       ← запись сделок и экспорт в Notion
__init__.py

📌 Обновлено (v1.1): добавлены auto/manual режимы, визуализация и отчётность вынесены в отдельный subflow Prefect.
...
plotly_engine.py          ← графики OHLC, уровни, зоны
dash_backtester_app.py    ← Dash-приложение для ручного режима
annotation_service.py     ← аннотации GPT Vision
prompt_formatter.py       ← форматирование промптов Vision
vision_interpreter.py     ← взаимодействие с GPT Vision / LangChain
visual_output_handler.py  ← сохранение PNG/JSON
__init__.py

📌 Обновлено (v1.1): расширен модуль, добавлены подмодули Vision Interpreter и Prompt Formatter, визуализация выполняется через Plotly/Dash.
...
prompt_manager.py         ← шаблоны и версии промптов
vision_interpreter.py     ← GPT Vision анализ изображений
annotation_service.py     ← аннотации графиков
decision_reconciler.py    ← объединение анализа LLM и Python
api_adapter.py            ← связь с OpenAI/Gemini API
memory_manager.py         ← контекстная память LangChain
embeddings.py             ← текст+изображение эмбеддинги
__init__.py

📌 Обновлено (v1.1): добавлены подмодули Vision Interpreter и Decision Reconciler, LangChain используется как инфраструктура LLM.

(эти файлы — сердце твоего “Learning Module”)  

🧬 9. Learning / Memory Subsystem  
```
Папка: src/ml_service/

api_server.py       ← REST API для взаимодействия с Prefect
vector_db.py        ← ChromaDB, хранение эмбеддингов
trainer.py          ← обучение моделей
scorer.py           ← скоринг результатов
__init__.py

📌 Обновлено (v1.1): ML вынесен в отдельный REST-сервис, взаимодействие через HTTP API.
...
streamlit_app.py          ← админ-панель (управление флоу Prefect)
components/
├── control_panel.py      ← общие элементы управления
├── flow_status.py        ← мониторинг состояния флоу
├── results_viewer.py     ← результаты и отчёты
└── __init__.py
__init__.py

📌 Обновлено (v1.1): Streamlit теперь только административный UI, визуализация выполняется в Dash, связь через FastAPI backend.
...
live_flow.py              ← лайв-анализ и сигналы
train_model_flow.py       ← обучение и обновление моделей
__init__.py

📌 Обновлено (v1.1): добавлен train_model_flow, все флоу используют JSON контракты, retry и Prefect Orion UI.
