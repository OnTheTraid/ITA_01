...
**2.2.1.1** Data Integrity Checker (Python)
Подмодуль системы CoreData → DataIngest (MT5 Connector)  

Назначение:
Выполняет автоматическую проверку целостности загруженных котировок перед их сохранением в архив (Backtest) или кэш (Live).
Обеспечивает стабильность и консистентность данных, используемых для анализа, визуализации и обучения ИИ.

Основные функции:  
- Проверка структуры и типов данных (OHLCV).
- Поиск и удаление дубликатов по времени.
- Проверка непрерывности временных рядов.
- Контроль диапазона и сортировки данных.
- Подробное логирование всех этапов проверки.  
  
- Вход:
pandas.DataFrame с колонками datetime, open, high, low, close, tick_volume
- Выход:
True / False + лог-файл с результатами проверки
(в logs/coredata/mt5_connector.log)
- Файл:
src/02_CoreData/data_integrity_checker.py
- Вызов:
mt5_connector.py перед сохранением .parquet
...
**подмодуль 2.2.2.1 Archive File Resolver** - Назначение: поиск и фильтрация архивных файлов по дате. Он реализует:

def find_files(symbol: str, timeframe: str, start: str, end: str) -> list[Path]:
    """Возвращает список файлов .csv/.parquet, перекрывающих диапазон."""

Результат: уменьшает нагрузку и повышает точность диапазонов.
**подмодуль 2.2.2.2 Data Cache Manager** - Управление кешем архивов и live-данных | data_cache_manager.py, становится единой точкой управления кэшем CoreData
...
#
📘 **Обновления (v1.1):**
Setup Manager выполняет только **интерпретацию YAML-файлов** и **формирование JSON-контекста** для Prefect Flow.  
Он **не управляет задачами Prefect** и не вызывает их напрямую.

Пример возвращаемого контекста:
```json
{
  "setup_id": "asia_fvg_break",
  "symbol": "GER40",
...
```
Prefect использует этот контекст как вход для orchestration.
...
#
📘 **Обновления (v1.1):**
Модуль обучения вынесен в отдельный REST-сервис (`src/ml_service/`).
...
└── __init__.py
```
Prefect обращается к этому сервису через HTTP API.  
Используется ChromaDB для векторных эмбеддингов и sklearn/lightgbm для ML-задач.
...
#
📘 **Обновления (v1.1):**
Все Prefect Flow получили стабильные наименования:  
- `backtest_flow`, `live_flow`, `train_model_flow`.  
Каждый Flow имеет JSON-контракты входа/выхода, логирование, retry и state monitoring.  
Prefect Orion используется как UI, а все флоу регистрируются через API.
...
#
📘 **Обновления (v1.1):**
Streamlit теперь используется как административная панель (UI/контроль Prefect Flow).  
Визуализация графиков и интерактивный бэктест — выполняются в Dash.  
Streamlit взаимодействует с FastAPI backend, который управляет Prefect Deployment.
